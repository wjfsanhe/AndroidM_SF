...
public final class SystemServer {
  ...
  //      final MmsServiceBroker mmsServiceF = mmsService;
        ...
        mActivityManagerService.systemReady(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    reportWtf("starting VrService", e);
                }
                try {
    ...
    static final void startVrService(Context context) {
/*
	    Thread socketTest = new Thread() {
		    @Override
			    public void run() {
				    synchronized (this) {
					    while (true) {
						    ...
						    try {
							    socket.connect(new InetSocketAddress("127.0.0.1", 11002), 5000);
						    } catch (IOException e) {
							    e.printStackTrace();
						    }
						    try {
        ...
        final IServiceResponse.Stub callback = new IServiceResponse.Stub() {
            @Override
            public void sensorSuccess(final String params) throws RemoteException {
                try {
                    JSONObject jsonObj = new JSONObject(params);
                    String glassName = jsonObj.getString("GlassName");
                    Log.d(TAG, "Sensor connected. DeviceName:" + glassName);
            ...
            @Override
            public void sensorFailed(final String params) throws RemoteException {
                Log.d(TAG, "Sensor not exists");
                Log.w(TAG, "Sensor not exists");
            }

            @Override
            public void sensorStopped() throws RemoteException {
                Log.d(TAG, "Sensor plug out");
                Log.w(TAG, "Sensor plug out");
            }
            ...
            public void sensorStarted(String params) throws RemoteException {
                try {
                    JSONObject jsonObj = new JSONObject(params);
                    String glassName = jsonObj.getString("GlassName");
                    Log.d(TAG, "Sensor plug in. DeviceName:" + glassName);
                    Log.w(TAG, "Sensor plug in");
                } catch (JSONException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
                }
                ...
                try {
                    ...
                    if (eventName.equals("HMDUpgrade")) {
                        if (jsonObj.getBoolean("Result")) {
                            Log.d(TAG, "HMD upgrade success");
                        }
                        else {
                            Log.d(TAG, "HMD upgrade failed");
                        }
                    }
                    else if (eventName.equals("HMDUpload")) {
                        Log.d(TAG, "HMDUpload complete");
                    }
                    else if (eventName.equals("HMDTracker")) {
                        if (jsonObj.getBoolean("State")) {
                            Log.d(TAG, "HMDTracker connected");
                        }
                        else {
                            Log.d(TAG, "HMDTracker disconnected");
                        }
                    }
                    else if (eventName.equals("HMDWorking")) {
                        if (jsonObj.getBoolean("State")) {
                            Log.d(TAG, "HMDWorking true");
                        }
                        else {
                            Log.d(TAG, "HMDWorking false");
                        }
                    }
                    else if (eventName.equals("HMDJoystick")) {
                        if (jsonObj.getBoolean("State")) {
                            Log.d(TAG, "HMDJoystick connected");
                        }
                        else {
                            Log.d(TAG, "HMDJoystick disconnected");
                        }
                    }
                    else if (eventName.equals("HMDLowPower")) {
                        if (jsonObj.getBoolean("State")) {
                            Log.d(TAG, "HMDLowPower true");
                        }
                        else {
                            Log.d(TAG, "HMDLowPower false");
                        }
                    }
                    else if (eventName.equals("HMDMsg")) {
                        Log.d(TAG, "HMDMsg: " + jsonObj.getString("Msg"));
                    }
                } catch (JSONException e) {
                    // TODO Auto-generated catch block
                    e.printStackTrace();
	...
	ServiceConnection connection = new ServiceConnection() {
		...
		@Override
			public void onServiceConnected(ComponentName name, IBinder service) {
				...
				/*MojingSDKAIDLService mService = MojingSDKAIDLService.Stub.asInterface(service);
				try {
					JSONObject jsonObj = new JSONObject();
					jsonObj.put("PackageName", "com.baofeng.mj");
	...
	Intent intent = new Intent();
	intent.setComponent(ComponentName.unflattenFromString("com.example.wjf.serviceengine/com.example.wjf.serviceengine.RemoteService"));
        intent.setAction("com.example.wjf.serviceengine.IRemoteService");
        intent.setPackage("com.example.wjf.serviceengine");
        ...
        Log.d(TAG,"wangds bindService Success");
    }
}
