...
public abstract class IntentResolver<F extends IntentFilter, R extends Object> {
    final private static String TAG = "ActivityManager";
    ...
    private void buildResolveList(Intent intent, FastImmutableArraySet<String> categories,
        ...
        int i;
        F filter;
	//filter is ServiceIntent info here we want.
        //for (i=0; i<N && (filter=src[i]) != null; i++) {
        for (i=0; i<N && (filter=src[i]) != null; i++) {
		filter= src[i] ;
		if(filter == null) {
			Slog.v(TAG, " filter is null  ");
			break;
		}
	    int match;
	    //filter is  ServiceIntentInfo{a19b17f com.baofeng.mj/com.baofeng.mojing.service.MojingSDKService}
        ...
        if (hasNonDefaults) {
            if (dest.size() == 0) {
                ...
                Slog.w(TAG, "too bad hasNonDefaults = " + hasNonDefaults );
	}
    }
