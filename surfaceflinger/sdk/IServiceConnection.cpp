...
status_t BnServiceConnection::onTransact(uint32_t code,const Parcel& data,Parcel* reply,uint32_t flags){
		...
		if(binder!=NULL &&  binder->remoteBinder() != NULL) {
			
			int32_t status = binder->linkToDeath(
					static_cast<IBinder::DeathRecipient*>(this));
			if (status != NO_ERROR) {
				SF_LOG(TAG, "serviceConnection : linkToDeath failed: %s (%d)",
						strerror(-status), status);
			}else{
				SF_LOG(TAG, "serviceConnection link to remote death success");
			}
		}
		return NO_ERROR;
