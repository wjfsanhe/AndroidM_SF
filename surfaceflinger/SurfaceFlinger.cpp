...
#include "sdk/TrackerThread.h"
#include "sdk/HeadTrackInputEventThread.h"
#include "./sdk/VrService.h"
...
void SurfaceFlinger::init() {
    ...
    mTrackerThread = new TrackerThread(this);
    mTrackerThread->setLuciferMode(true);
    mVrService = new VrService(this);
    //mVrService->setServerPort(mTrackerThread->getServerPort());
    mHeadTrackInputEventThread = new HeadTrackInputEventThread();
...
bool SurfaceFlinger::isBootFinished(){
        return mBootFinished;
}
// ----------------------------------------------------------------------------
...
void SurfaceFlinger::setUpHWComposer() {
    ...
    needsBFRender = false;//baofeng  setting bootanimation   input method
        ...
        const Region& inDirtyRegion)
{
    ...
    if (CC_LIKELY(!mDaltonize && !mHasColorMatrix)) {
        if(needsBFRender)
        {
            ...
            RenderEngine& engine(getRenderEngine());
            engine.bindTmpFBO();
            float pfViewMatrix[16];
            mTrackerThread->getLastHeadView(pfViewMatrix);
            //mTrackerThread->getLastHeadQuarternion(&engine.mRotation.w, &engine.mRotation.x, &engine.mRotation.y, &engine.mRotation.z);

            engine.setViewMatrix(pfViewMatrix);
            mHeadTrackInputEventThread->beginTracker();
            if(!doComposeSurfaces(hw, dirtyRegion)) return;
            mHeadTrackInputEventThread->endTracker(pfViewMatrix);
            //distortion for VR
            engine.drawDistortion();
        } else{
            if (!doComposeSurfaces(hw, dirtyRegion)) return;
        }
    } else {
...
void SurfaceFlinger::setViewMatrix(float sdkMatrix[])
{
        memcpy(mFViewMatrix, sdkMatrix,sizeof(mFViewMatrix));
        invalidateHwcGeometry();
}
...
int32_t SurfaceFlinger::getServerPort() {
	if(mTrackerThread!=NULL){
		return mTrackerThread->getServerPort();
	}
	return 0;
