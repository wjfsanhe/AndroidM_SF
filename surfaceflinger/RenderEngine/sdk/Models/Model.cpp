...
namespace Baofeng
{
	namespace Mojing
	{
		...
		void Model::prepareData()
		{
			...
			for (int iIndex = 0 ; iIndex < sizeof(fDefaultNormals) / sizeof(float) ; iIndex++)
				data.normals.push_back(fDefaultNormals[iIndex]);
		...
		Model *Model::CreateModel(MODEL_TYPE ModelType, TEXTURE_TYPE TextureType)
		{
			Model * pRet = NULL;
			switch (ModelType)
			{
			...
			case MODEL_RECT_2D:
			{
				pRet = new RectangleModel(ModelType);
					break;
			}
			default:
				break;
